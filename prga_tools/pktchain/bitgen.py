...
class PktchainBitgen(object):
    ...
    def bitgen(cls,
            ...
            , max_packet_frames = 0     # maximum number of frames per packet
            ):
        ...
        if max_packet_frames == 0:
            for y, x in product(reversed(range(len(bits[0]))), reversed(range(len(bits)))):
                ...
                for i in reversed(range(len(bitstream) // 32)):
                    ostream.write("{:0>8x}".format(
            ...
            for pkt in count():
                ...
                for y, x in product(reversed(range(len(bits[0]))), reversed(range(len(bits)))):
                    ...
                    completed = completed and not checksum
...
if __name__ == '__main__':
    ...
    parser.add_argument('-M', '--max_frames_per_packet', type=int, default=0, dest='max_packet_frames',
            help="Maximum number of 32b frames per packet. By default no limit set")
