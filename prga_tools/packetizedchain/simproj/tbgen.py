...
def generate_testbench_wrapper(context, template, ostream, tb_top, behav_top, io_bindings):
    ...
    bs_size = 0
    for range_ in packetizedchain_hop2ranges(context):
        for hoc_head, effective_payload, hoc_tail in packetizedchain_packetize(range_):
            bs_size += 48                       # header
            bs_size += 32 if hoc_head else 0    # HOC head
            bs_size += effective_payload
            bs_size += 32 if hoc_tail else 0    # HOC tail
