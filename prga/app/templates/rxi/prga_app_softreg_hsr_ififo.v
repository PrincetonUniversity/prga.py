...
module prga_app_softreg_hsr_ififo #(
    ...
    always @(posedge clk) begin
        if (~rst_n) begin
            resp_vld <= 1'b0;
        end else if ((req_rdy && req_vld && !(req_we && &req_wmask)) || (var_wr && !var_full)) begin
            resp_vld <= 1'b1;
    ...
    assign req_rdy = !var_wr && !resp_vld;

    generate if (DATA_WIDTH == 1) begin
        assign resp_data = var_full;
    end else begin
        assign resp_data = { { (DATA_WIDTH - 1) {1'b0}}, var_full };
    end endgenerate
