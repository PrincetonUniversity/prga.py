...
module prga_rxi_fe #(
    ...
    // -- clock divider --
    reg                                 clkgen_period_p1x100ps_we;
    wire [31:0]                         clkgen_period_p1x100ps;

    prga_byteaddressable_reg #(
        .NUM_BYTES      (4)
        ,.RST_VALUE     (32'd99)    // default: 100MHz
    ) i_clkgen_period_p1x100ps (
        .clk            (clk)
        ,.rst           (~rst_n)
        ,.wr            (clkgen_period_p1x100ps_we)
        ,.mask          (s_req_strb[0+:4])
        ,.din           (s_req_data[0+:32])
        ,.dout          (clkgen_period_p1x100ps)
        );

    prga_fake_clkgen i_clkgen (
        .period_p1x100ps_i  (clkgen_period_p1x100ps)
        ,.gclk              (aclk)
        );
    ...
    always @* begin
        ...
        clkgen_period_p1x100ps_we = 1'b0;
        ...
        // ------------
            ...
            if (s_req_strb[0]) begin
                buffer_bogus( prq_full, s_req_vld );
                clkgen_period_p1x100ps_we = s_req_vld && !prq_full;
            end

            // buffer load response
            else
                buffer_response(clkgen_period_p1x100ps, prq_full, rb_full, s_req_vld);
