...
module prga_yami_pitoncache_rpb (
    ...
    , input wire [`PRGA_YAMI_MTHREAD_ID_WIDTH-1:0]              rpb_thread_id_s2
    ...
    , input wire [`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2-1:0]    rpb_rob_entry_s2

    // -- Stage III --
    , input wire                                                enqueue_rpb_s3
    , input wire [`PRGA_YAMI_REQTYPE_WIDTH-1:0]                 rpb_reqtype_s3
    , input wire [`PRGA_YAMI_MTHREAD_ID_WIDTH-1:0]              rpb_thread_id_s3
    ...
    , output wire [`PRGA_YAMI_MTHREAD_ID_WIDTH-1:0]             rpb_thread_id_s1
    ...
    // -- buffer S2 request --
    reg                                             s2buf_vld;
    reg [`PRGA_YAMI_MTHREAD_ID_WIDTH-1:0]           s2buf_thread_id;
    ...
    reg [`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2-1:0] s2buf_rob_entry;

    always @(posedge clk) begin
        if (~rst_n) begin
            s2buf_vld       <= 1'b0;
            s2buf_thread_id <= { `PRGA_YAMI_MTHREAD_ID_WIDTH {1'b0} }; 
    ...
    always @(posedge clk) begin
        if (~rst_n) begin
            s3buf_vld       <= 1'b0;
            s3buf_thread_id <= { `PRGA_YAMI_MTHREAD_ID_WIDTH {1'b0} }; 
            ...
            s3buf_vld       <= 1'b0;
            s3buf_thread_id <= rpb_thread_id_s3;
    ...
    assign rpb_thread_id_s1 = state == ST_S2_BUFFERED ? s2buf_thread_id : s3buf_thread_id;
    ...
    assign rpb_rob_entry_s1 = state == ST_S2_BUFFERED ? s2buf_rob_entry : s3buf_rob_entry;
