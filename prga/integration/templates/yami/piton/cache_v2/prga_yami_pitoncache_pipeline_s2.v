...
module prga_yami_pitoncache_pipeline_s2 (
    ...
    , input wire [`PRGA_YAMI_CACHE_NUM_WAYS_LOG2-1:0]           inv_prq_way_s2_next
    , input wire [`PRGA_YAMI_MTHREAD_ID_WIDTH-1:0]              thread_id_s2_next
    ...
    , output reg [`PRGA_YAMI_MTHREAD_ID_WIDTH-1:0]              thread_id_s2    // also thread_id_s3_next
    , output reg [`PRGA_YAMI_CACHE_NUM_WAYS_LOG2-1:0]           inv_prq_way_s2  // also inv_prq_way_s3_next
    , output reg [`PRGA_YAMI_REQTYPE_WIDTH-1:0]                 reqtype_s2      // also reqtype_s3_next
    , output reg [`PRGA_YAMI_SIZE_WIDTH-1:0]                    size_s2         // also size_s3_next
    , output reg [`PRGA_YAMI_FMC_ADDR_WIDTH-1:0]                addr_s2         // also addr_s3_next
    , output reg [`PRGA_YAMI_MFC_DATA_WIDTH-1:0]                data_s2         // also data_s3_next
    , output reg [`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2-1:0]    rob_entry_s2    // also rob_entry_s3_next
    ...
    always @(posedge clk) begin
        if (~rst_n) begin
            ...
            thread_id_s2   <= { `PRGA_YAMI_MTHREAD_ID_WIDTH {1'b0} };
            inv_prq_way_s2 <= { `PRGA_YAMI_CACHE_NUM_WAYS_LOG2 {1'b0} };
            ...
            op_s2          <= op_s2_next;
            thread_id_s2   <= thread_id_s2_next;
            inv_prq_way_s2 <= inv_prq_way_s2_next;
