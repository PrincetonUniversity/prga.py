...
module prga_yami_pitoncache_rob (
    ...
    // -- Ports to Pipeline --------------------------------------------------
    // -- Stage I --
    , output wire                                               next_entry_vld_s1
    , output wire [`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2-1:0]   next_entry_s1

    , input wire                                                alloc_s1
    , input wire [`PRGA_YAMI_RESPTYPE_WIDTH-1:0]                alloc_resptype_s1
    ...
    always @(posedge clk) begin
        if (~rst_n) begin
            rptr            <= { (`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2 + 1) {1'b0} };
            wptr            <= { (`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2 + 1) {1'b0} };
        end else begin
            if (alloc_s1 && next_entry_vld_s1) begin
                entries[wptr[0+:`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2]] <= { 1'b0, alloc_resptype_s1, {`PRGA_YAMI_MFC_DATA_WIDTH {1'b0} } };
                wptr <= wptr + 1;
            end
    ...
    always @* begin
        ...
        else if (fill_s3 && fill_entry_s3[0 +: `PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2] == rptr) begin
            mfc_vld = 1'b1;
    ...
    assign next_entry_vld_s1 = wptr != {~rptr[`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2], rptr[0+:`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2]};
    assign next_entry_s1 = wptr[0+:`PRGA_YAMI_CACHE_ROB_NUM_ENTRIES_LOG2];
