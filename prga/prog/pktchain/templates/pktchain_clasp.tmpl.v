...
{%- set chain_width = module.chain_width %}
`include "pktchain.vh"
`timescale 1ns/1ps
module pktchain_clasp (
    ...
    always @(posedge prog_clk) begin
        if (~frame_empty && frame_rd && ctrl_init) begin
            ...
            {% for i in range(chain_width) %}
            crc[{{ i }}] <= (crc[{{ i }}] << 1) ^ (crc[{{ i }}][7] ^ prog_dout[{{ i }}] ? 8'h07 : 8'b0);
            {%- endfor %}
        end
    end
    ...
    // The scanchain width ({{ chain_width }}) is unsupported (Accepted values: 1, 2, 4)
    __PRGA_RTLGEN_ERROR__ __PKTCHAIN_UNSUPPORTED_CHAIN_WIDTH__();
    {%- endif %}
