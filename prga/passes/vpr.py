...
class VPRScalableDelegate(Object):
    ...
    def add_active_tile(self, block, ori = None, fc = None):
        """Add a tile into the output VPR specs.

        Args:
            block (`Module`): An IO or logic block
            ori (`Orientation`): If ``block`` is an IO block, specify the edge on which the IO block should be placed
            fc (`BlockFCValue`): FC values for the block
        """
    ...
    def add_layout_rule(self, rule, priority, block, ori = None, **kwargs):
        ...
        Args:
            ...
            block (`Module`): The block type for the rule. Use ``None`` to explicitly add rules for empty tiles.
            ori (`Orientation`): If ``block`` is an IO block, specify the edge on which the IO block should be placed
