...
class VPRInputsGeneration(Object, AbstractPass):
    ...
    def _pb_type(self, module, hierarchy = None):
        ...
        with self.xml.element("pb_type", attrs):
            ...
            for port in itervalues(module.ports):
                ...
                if getattr(port, "equivalent", False):
    ...
    def _rrg_tile_pinlist(self, port, subblock_name, srcsink_ptc, iopin_ptc):
        equivalent = getattr(port, "equivalent", False)
        if equivalent:
            ...
            for subblock in range(block.capacity):
                ...
                for port in itervalues(block.ports):
                    if port.direction.is_input and not port.is_clock:
                        if subblock == 0:
                            blockpin2ptc[port.key] = srcsink_ptc, getattr(port, "equivalent", False), iopin_ptc
                        srcsink_ptc, iopin_ptc = self._rrg_tile_pinlist(port, subblock_name, srcsink_ptc, iopin_ptc)
                for port in itervalues(block.ports):
                    if port.direction.is_output:
                        if subblock == 0:
                            blockpin2ptc[port.key] = srcsink_ptc, getattr(port, "equivalent", False), iopin_ptc
    ...
    def _rrg_node(self, type_, id_, ptc, xlow, ylow, *,
            track_dir = None, port_ori = None, xhigh = None, yhigh = None, segment_id = None, capacity = 1):
