...
def _traverse_single_widechain(context, tail, bitmap, bitcounts, drop_cache):
    ...
    while True:
        ...
        if prev.net_type.is_port:   # we reached the head of the chain
            assert prev.direction.is_input and prev.name == head and prev.net_class.is_config
            break
