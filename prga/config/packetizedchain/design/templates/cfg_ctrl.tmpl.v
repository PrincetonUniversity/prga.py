...
{%- set width = module.config_width %}
module {{ module.name }} (
    input wire [0:0] cfg_clk,
    input wire [0:0] cfg_e,

    input wire [{{ width - 1 }}:0] cfg_i,
    output reg [{{ width - 1 }}:0] cfg_o,
    ...
    localparam  MAGIC_SOP = {{ magic_sop }};    // magic number for start of packet
    ...
    always @(posedge cfg_clk) begin
        if (cfg_e) begin
            buffer <= 32'b0;
            bit_count <= 16'b0;
            cfg_o <= {{ '{' -}} CFG_WIDTH {{- '{' -}} 1'b0 {{- '}}' -}};
            state <= ST_IDLE;
        end begin
            if (en_output) begin
                {{ '{' -}} cfg_o, buffer {{- '}' }} <= {{ '{' -}} buffer_next, cfg_i {{- '}' }};
            end else begin
                cfg_o <= {{ '{' -}} CFG_WIDTH {{- '{' -}} 1'b0 {{- '}}' -}};
                buffer <= {{ '{' -}} buffer_next, cfg_i {{- '}' }};
            end
